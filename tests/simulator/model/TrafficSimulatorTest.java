...
class TrafficSimulatorTest {
	...
	void test_1() {
		...
		assertTrue(new JSONObject(s).similar(new JSONObject(d)));

		ts.advance();
		ts.advance();
		String e = ts.report().toString();
		...
		assertTrue(new JSONObject(s).similar(new JSONObject(e)));
